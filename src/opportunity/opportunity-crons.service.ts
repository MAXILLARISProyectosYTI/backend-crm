...
export class OpportunityCronsService {
    ...
    try {
     ...
     // Procesar oportunidades una por una para evitar problemas de concurrencia
      ...
      try {
        ...
        if (opportunity) {
          const nextUserAssigned = await this.getNextUserToAssingOpportunityWithTracking(
            opportunity.cSubCampaignId || '', 
            lastAssignedUsersBySubcampaign
          );
          
          if (!nextUserAssigned) {
            console.error(`❌ No se pudo obtener usuario para asignar oportunidad ${opportunity.id}`);
            errorCount++;
            continue;
          }
          ...
          lastAssignedUsersBySubcampaign.set( opportunity.cSubCampaignId || '', nextUserAssigned.id);
          successCount++;
          
        } 
      } catch (error) {
        console.error(`❌ Error asignando oportunidad ${opportunity.id}:`, error);
        errorCount++;
  ...
  async getNextUserToAssingOpportunityWithTracking(
    ...
    try {
      let listUsers = await this.userService.getUsersBySubCampaignId(subCampaignId);
      ...
      if(listUsers.length === 0) {
        ...
        switch(subCampaignId) {
          case CAMPAIGNS_IDS.OI:
            usersDefault = await this.userService.getUserByAllTeams([TEAMS_IDS.EJ_COMERCIAL_OI]);
            break;
          case CAMPAIGNS_IDS.OFM:
            usersDefault = await this.userService.getUserByAllTeams([TEAMS_IDS.TEAM_LEADERS_COMERCIALES]);
            break;
          case CAMPAIGNS_IDS.APNEA:
            usersDefault = await this.userService.getUserByAllTeams([TEAMS_IDS.EJ_COMERCIAL_APNEA]);
            break;
          default:
            console.error(`❌ Subcampaña no reconocida para asignación por defecto: ${subCampaignId}`);
            return null;
        }

        const userSelected = usersDefault[Math.floor(Math.random() * usersDefault.length)];
        const userDetails = await this.userService.findOne(userSelected.user_id);
      ...
      if (lastAssignedUserId) {
        ...
        lastOpportunityAssigned = await this.opportunityService.getLastOpportunityAssigned(subCampaignId);
      }
      ...
      if (!nextUser) {
        console.error(`❌ No se pudo determinar el siguiente usuario para subcampaña: ${subCampaignId}`);
        return listUsers[0]; // Fallback al primer usuario
      }

      return nextUser;
    } catch (error) {
      console.error(`❌ Error en getNextUserToAssingOpportunityWithTracking para subcampaña ${subCampaignId}:`, error);
      return null;
    }
  }
  ...
  async assignOpportunityWithTransaction(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        await this.opportunityRepository.manager.transaction(async transactionalEntityManager => {
          try {
            ...
            try {
              await this.websocketService.notifyOpportunityUpdate(opportunity, nextUserAssigned.id);
            } catch (notificationError) {
              // Si falla la notificación, log el error pero no fallar la transacción
              console.error(`⚠️ Error enviando notificación para oportunidad ${opportunity.id}:`, notificationError);
            }

          } catch (error) {
            console.error(`❌ Error en transacción para oportunidad ${opportunity.id} (intento ${attempt}):`, error);
        ...
        if (attempt === maxRetries) {
          console.error(`❌ Error definitivo después de ${maxRetries} intentos para oportunidad ${opportunity.id}:`, error);
          throw error;
        }
